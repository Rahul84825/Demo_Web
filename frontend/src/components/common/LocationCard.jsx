 ...
 * Professional premium styling matching DemoMart brand
 */
const LocationCard = ({ compact = false }) => {
  const storeInfo = {
    ...
    addressLine3: "Demo Address Line 3",
    city: "Demo City, Demo City",
    postalCode: "Demo State 000000",
    phone: "+91 99999 99999",
    phoneRaw: "9999999999",
    email: "admin@demomart.com",
    mapsUrl: "https://maps.google.com/?q=product+World+Viman+Nagar+Demo City",
  };
  ...
  const ActionButton = ({ icon: Icon, label, href, type = "primary" }) => {
    ...
    const variants = {
      primary:
        "bg-[var(--charcoal)] text-white hover:bg-[#2a1410]",
      success:
        "bg-[#22c55e] text-white hover:bg-[#16a34a]",
      outline:
        "border border-[rgba(99, 102, 241,0.15)] bg-white text-[#0F172A] hover:bg-slate-50",
    };
  ...
  if (compact) {
    ...
    return (
      <div className="rounded-xl border border-[rgba(99, 102, 241,0.10)] bg-white p-4 shadow-sm">
        <div className="flex items-start gap-3 mb-3">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg
                          border border-[rgba(99, 102, 241,0.20)] bg-[rgba(99, 102, 241,0.08)]">
            <MapPin className="h-3.5 w-3.5 text-[#6366F1]" />
          </div>
          <div className="min-w-0">
            <p className="text-xs font-medium uppercase tracking-wider text-[rgba(99, 102, 241,0.40)]">
              Store Location
            </p>
            <p className="text-sm font-medium text-[#0F172A] mt-1">{storeInfo.name}</p>
            <p className="text-xs text-[#475569] leading-snug mt-0.5">
  ...
  return (
    <div className="rounded-2xl border border-[rgba(99, 102, 241,0.10)] bg-white p-8 shadow-sm">
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-lg font-medium text-[#0F172A]">{storeInfo.name}</p>
          </div>
        </div>

        <div className="space-y-1.5 ml-0 text-sm leading-relaxed text-[#475569]">
          <p>{storeInfo.address}</p>
          <p>{storeInfo.addressLine2}</p>
          <p>{storeInfo.addressLine3}</p>
          <p className="font-medium text-[#0F172A]">{storeInfo.city}</p>
          <p className="text-xs font-medium text-[#9c6a18]">{storeInfo.postalCode}</p>
        </div>
      </div>

      {/* Contact details */}
      <div className="my-6 border-t border-[rgba(99, 102, 241,0.07)] pt-6">
        <h4 className="text-sm font-medium text-[#0F172A] mb-3">Quick Contact</h4>
      ...
      {/* Action buttons */}
      <div className="grid grid-cols-2 gap-3 pt-2 border-t border-[rgba(99, 102, 241,0.07)]">
        <ActionButton
          icon={Phone}
          label="Call Now"
          href={`tel:+91${storeInfo.phoneRaw}`}
