  ...
  return (
    <div className="max-w-4xl mx-auto page-enter pb-20">
      <div className="flex items-center gap-4 mb-8">
        <button onClick={() => navigate("/admin/products")} className="p-2 hover:bg-slate-100 rounded-full transition-colors"><ArrowLeft size={20} /></button>
        <div className="section-title mb-0">
          <h2 className="">{isEditMode ? "Edit product" : "Add New product"}</h2>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-200 space-y-4 shadow-sm">
            <div>
              <label className="text-[10px] font-medium uppercase tracking-widest text-slate-500 mb-1.5 block">Product Name</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-[10px] font-medium uppercase tracking-widest text-slate-500 mb-1.5 block">Category</label>
              ...
              <div>
                <label className="text-[10px] font-medium uppercase tracking-widest text-slate-500 mb-1.5 block">Collection Type</label>
                <select 
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-[10px] font-medium uppercase tracking-widest text-slate-500 mb-1.5 block">GST % (Info only)</label>
                <input type="number" value={form.gstPercent} onChange={e => set("gstPercent", e.target.value)} className="input-field" />
              </div>
              <div>
                <label className="text-[10px] font-medium uppercase tracking-widest text-slate-500 mb-1.5 block">Packing Charges (₹)</label>
            ...
            <div>
              <label className="text-[10px] font-medium uppercase tracking-widest text-slate-500 mb-1.5 block">Tags (Optional)</label>
              <input value={form.tags} onChange={e => set("tags", e.target.value)} className="input-field" placeholder="e.g. sugarfree, healthy" />
            </div>

            <div>
              <label className="text-[10px] font-medium uppercase tracking-widest text-slate-500 mb-1.5 block">Description</label>
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              <label className="text-[10px] font-medium uppercase tracking-widest text-slate-500">Pricing & Variants</label>
              <button onClick={addVariant} className="text-[10px] font-medium uppercase tracking-widest text-blue-600 hover:underline flex items-center gap-1"><Plus size={12} /> Add Size</button>
            </div>
            ...
            <div className="space-y-4">
              {form.variants.map((v, i) => {
                ...
                return (
                  <div key={v.id} className={`p-4 rounded-2xl border relative group transition-colors ${v.isAvailable ? 'bg-slate-50/30 border-slate-200' : 'bg-gray-50 border-gray-200'}`}>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      <div>
                        <label className="text-[9px] font-medium text-slate-500 mb-1 block">Label (e.g. 500g)</label>
                      ...
                      <div>
                        <label className="text-[9px] font-medium text-slate-500 mb-1 block">MRP (₹)</label>
                      ...
                      <div>
                        <label className="text-[9px] font-medium text-slate-500 mb-1 block">Discount (%)</label>
                    ...
                    <div className="flex items-center justify-end gap-3 mt-4 pt-4 border-t border-slate-200/50">
                    ...
                    <div className="mt-4 flex flex-col gap-2 border-t border-slate-200 pt-3">
                      <div className="flex justify-between items-center text-[10px]">
                         <span className="text-slate-500 font-medium uppercase tracking-tighter">Price Breakdown</span>
                         <span className="text-slate-500 italic">{TAX_MESSAGE}</span>
                      </div>
                      
                      <div className="grid grid-cols-4 gap-2">
                         <div className="bg-white/50 p-2 rounded-lg border border-slate-200/50">
                            <p className="text-[8px] font-bold text-slate-500 uppercase tracking-tighter mb-0.5">Net Selling</p>
                            <p className="text-xs font-bold text-slate-900">
                               {formatCurrency(sp)}
                            </p>
                         </div>
                         <div className="bg-white/50 p-2 rounded-lg border border-slate-200/50">
                            <p className="text-[8px] font-bold text-slate-500 uppercase tracking-tighter mb-0.5">GST ({form.gstPercent}%)</p>
                            <p className="text-xs font-bold text-emerald-600">
                               + {formatCurrency(Math.round((sp * Number(form.gstPercent || 0)) / 100))}
                            </p>
                         </div>
                         <div className="bg-white/50 p-2 rounded-lg border border-slate-200/50">
                            <p className="text-[8px] font-bold text-slate-500 uppercase tracking-tighter mb-0.5">Packing</p>
                            <p className="text-xs font-bold text-emerald-600">
                               + {formatCurrency(Number(form.packingCharges || 0))}
                            </p>
                         </div>
                         <div className="bg-blue-600/5 p-2 rounded-lg border border-blue-500/20">
                            <p className="text-[8px] font-bold text-blue-600 uppercase tracking-tighter mb-0.5">Total Price</p>
        ...
        <div className="space-y-8">
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
            <label className="text-[10px] font-medium uppercase tracking-widest text-slate-500 mb-4 block">Product Images</label>
            
            <div className="space-y-4">
              <div 
                onDragOver={onDragOver}
                onDrop={onDrop}
                className={`aspect-square rounded-2xl bg-slate-50 border-2 border-dashed overflow-hidden flex flex-col items-center justify-center relative group transition-colors ${errors.image ? 'border-red-300' : 'border-slate-200 hover:border-cyan-500'}`}
              >
                {form.images[0] ? (
                  <img src={form.images[0]} className="w-full h-full object-cover" alt="" />
                ) : (
                  <div className="text-center p-4">
                    <UploadCloud size={32} className="mx-auto text-slate-500 mb-2 opacity-50" />
                    <p className="text-[10px] font-medium text-slate-500 uppercase tracking-widest">Drop files here</p>
                    <p className="text-[9px] text-slate-500 mt-1 font-medium">or click to browse</p>
                  </div>
                )}
                ...
                {(uploading || uploadProgress > 0) && (
                  <div className="absolute inset-0 bg-white/90 flex flex-col items-center justify-center p-6 text-center">
                    ...
                    <p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest">Uploading {uploadProgress}%</p>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-4 gap-2">
                {form.images.map((img, i) => (
                  <div key={i} className="aspect-square rounded-lg bg-gray-100 overflow-hidden border border-slate-200 relative group cursor-pointer hover:border-cyan-500 transition-all">
                    ...
                    {img === form.image && <div className="absolute top-1 right-1 h-2 w-2 bg-cyan-500 rounded-full shadow-sm" />}
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-slate-900 p-6 rounded-3xl text-white shadow-2xl space-y-6">
            <h3 className=" text-xl text-cyan-400">Publish</h3>
            ...
            {/* Storefront Toggle */}
            <div className="flex items-center justify-between p-4 rounded-2xl bg-white/5 border border-white/10 group hover:border-cyan-500 transition-colors cursor-pointer" onClick={() => set("isActive", !form.isActive)}>
              <div className="flex items-center gap-3">
                <div className={`h-8 w-8 rounded-lg flex items-center justify-center transition-colors ${form.isActive ? 'bg-cyan-500 text-slate-900' : 'bg-white/10 text-white/40'}`}>
                  <Package size={16} />
                </div>
                <div>
            ...
            <button 
              ...
              className={`w-full h-14 rounded-2xl font-medium uppercase tracking-widest text-xs transition-all shadow-xl flex items-center justify-center gap-2
                ${saved ? 'bg-green-500' : 'bg-blue-600 hover:bg-cyan-400 hover:text-slate-900'}`}
            >
              {submitting ? 'Processing...' : saved ? <><CheckCircle2 size={18} /> Success</> : <><Save size={18} /> Save product</>}
            </button>
          </div>
        </div>
      </div>
