        ...
        [
          {
            ...
            items: itemSnapshots,
            payment: {
              method: orderData?.payment?.method || orderData?.paymentMethod || "demopayment",
              status: orderData?.payment?.status || "PENDING",
              gateway: orderData?.payment?.gateway || "",
              demopaymentOrderId: orderData?.payment?.demopaymentOrderId,
              demopaymentPaymentId: orderData?.payment?.demopaymentPaymentId,
              demopaymentSignature: orderData?.payment?.demopaymentSignature
            },
            status: "PLACED",
...
export const rejectOrder = async (req, res) => {
  ...
  try {
    ...
    const rzpPaymentId = order.payment?.demopaymentPaymentId;

    if (payMethod === "demopayment" && payStatus === "PAID" && rzpPaymentId) {
      const demopayment = getDemoPaymentClient();
      if (demopayment) {
        try {
          const total = Number(order.totals?.grandTotal || order.total || 0);
          if (total > 0) {
            const refund = await demopayment.payments.refund(rzpPaymentId, {
...
export const getOrdersByStatus = async (req, res) => {
  ...
  try {
    // Asynchronously sync active DemoDelivery orders in the background
