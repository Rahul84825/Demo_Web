...
export const paymentVerifyValidation = [
  body("demopayment_order_id").trim().isLength({ min: 1, max: 200 }).withMessage("Missing DemoPayment order id").notEmpty().withMessage("DemoPayment order id cannot be empty"),
  body("demopayment_payment_id").trim().isLength({ min: 1, max: 200 }).withMessage("Missing DemoPayment payment id").notEmpty().withMessage("DemoPayment payment id cannot be empty"),
  body("demopayment_signature").trim().isLength({ min: 1, max: 500 }).withMessage("Missing DemoPayment signature").notEmpty().withMessage("DemoPayment signature cannot be empty"),
...
export const categoryCreateValidation = [
  ...
  body("type").optional().trim().toLowerCase().isIn(["products", "other"]).withMessage("type must be one of: products, other"),
